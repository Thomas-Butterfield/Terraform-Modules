...
resource "azurerm_windows_web_app_slot" "webapp" {
  ...
  dynamic "storage_account" {
    ...
    content {
      ...
      value = storage_account.value.value
    }
  }

  dynamic "action" {
    for_each = lookup(var.settings, "action", {}) == {} ? [] : [1]

    content {
      action_type                    = var.settings.action.action_type
      minimum_process_execution_time = try(var.settings.action.minimum_process_execution_time, null)
    }
  }
  ...
  site_config {
    ...
    websockets                                    = try(var.settings.site_config.websockets, null)
    ...
    dynamic "application_stack" {
      ...
      content {
        ...
        docker_container_name     = try(var.settings.site_config.application_stack.docker_container_name, null)
        docker_container_registry = try(var.settings.site_config.application_stack.docker_container_registry, null)
        docker_container_tag      = try(var.settings.site_config.application_stack.docker_container_tag, null)
        dotnet_version            = try(var.settings.site_config.application_stack.dotnet_version, null)
        java_container            = try(var.settings.site_config.application_stack.java_container, null)
        java_container_version    = try(var.settings.site_config.application_stack.java_container_version, null)
      ...
      content {
        dynamic "action" {
          ...
          content {
            ...
            dynamic "custom_action" {
              for_each = lookup(var.settings.site_config.auto_heal_setting.action, "custom_action", {}) == {} ? [] : [1]

              content {
                executable = var.settings.site_config.auto_heal_setting.action.custom_action.executable
                parameters = try(var.settings.site_config.auto_heal_setting.action.custom_action.parameters, null)
              }
            }
          }
        }
        dynamic "trigger" {
          ...
          content {
            ...
            private_memory_kb = try(var.settings.site_config.auto_heal_setting.trigger.private_memory_kb, null)
