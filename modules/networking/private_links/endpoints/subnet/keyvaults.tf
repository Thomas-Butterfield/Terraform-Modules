module "keyvault" {
  ...
  resource_id         = can(each.value.resource_id) ? each.value.resource_id : var.remote_objects.keyvaults[try(each.value.keyvault_key, each.key)].id
  resource_name       = can(each.value.resource_name) ? each.value.resource_name : var.remote_objects.keyvaults[try(each.value.keyvault_key, each.key)].name
  settings            = each.value
