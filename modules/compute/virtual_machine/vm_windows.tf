...
resource "azurerm_windows_virtual_machine" "vm" {
  ...
  admin_password = coalesce(var.admin_password, try(each.value.admin_password, null), local.admin_password, try(random_password.admin[local.os_type].result, null))
  admin_username = coalesce(var.admin_username, try(each.value.admin_username, null), local.admin_username)
  ...
  source_image_id = try(each.value.custom_image_id, var.custom_image_ids[each.value.custom_image_key].id, var.custom_image_ids, null)
  ...
  lifecycle {
    ignore_changes = [
      resource_group_name, location, os_disk[0].name, availability_set_id, admin_password
    ]
  }
