...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  admin_password = try(coalesce(var.admin_password, try(each.value.admin_password, null)), null)
  admin_username = coalesce(var.admin_username, try(each.value.admin_username, null))
