...
locals {
  LegacyADLoginForWindows_name            = try(coalesce(var.ad_domain_name, try(var.extension.ad_domain_name, null)), null)
  LegacyADLoginForWindows_domain_username = try(coalesce(var.vm_domain_username, try(var.extension.domain_username, null), try(data.azurerm_key_vault_secret.LegacyADLoginForWindows_username["enabled"].value, null)), "DOMAIN_USERNAME_NOT_SET")
  LegacyADLoginForWindows_domain_password = try(coalesce(var.vm_domain_password, try(var.extension.domain_password, null), try(data.azurerm_key_vault_secret.LegacyADLoginForWindows_password["enabled"].value, null)), "DOMAIN_PASSWORD_NOT_SET")
}
