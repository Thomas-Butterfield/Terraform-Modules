...
data "azurerm_key_vault_secret" "promote_vm_domain_controller_domain_username" {
  for_each = var.extension_name == "Promote_VM_DomainController" && var.vm_domain_username == null && try(var.extension.domain_username, null) == null && var.ad_install_forest == false ? toset(["enabled"]) : toset([])
...
data "azurerm_key_vault_secret" "promote_vm_domain_controller_admin_password" {
  for_each = var.extension_name == "Promote_VM_DomainController" && var.vm_admin_password == null && try(var.extension.admin_password, null) == null ? toset(["enabled"]) : toset([])
