...
module "virtual_machine_groups_named" {
  source = "./modules/compute/virtual_machine_personal_named"
  for_each = {
    for key, value in try(local.settings.virtual_machine_groups, {}) : key => value
    if try(value.enabled, false) == true && try(value.named_vms, false) == true
  }
